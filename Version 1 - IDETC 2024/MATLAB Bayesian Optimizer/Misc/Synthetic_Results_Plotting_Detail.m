function [ax1, ax2] = Synthetic_Results_Plotting_Detail(min_loc,x_obj, y_obj, x_cnst, y_cnst, bnds,varargin)
...
ax1 = axes;
surf(ax1,x_p,y_p,reshape(fmu,size(x_p)),EdgeColor="none")
xlim(bnds(1,[1,2]))
ylim(bnds(2,[1,2]))
ax1.XTick = 4:2:30;
ax1.YTick = 200:10:280;
ax1.FontName = "Times";
ax1.FontWeight = "Bold";
ax1.FontSize = 14;
xlabel(ax1, "Carrying Capacity (Number)", "FontWeight","normal")
ylabel(ax1, "Operating Time (Minutes)", "FontWeight","normal")
...
surf(ax2,x_p,y_p,reshape(gmu,size(x_p)),'FaceAlpha',0.4,EdgeColor="none")
hold on
[M,c] = contour3(ax2,x_p,y_p,reshape(gmu,size(x_p)),50,'EdgeAlpha',0.5);
%[M,c] = contour3(ax2,x_p,y_p,reshape(-gmu_latent,size(x_p)),60);

%separate constraint values so we can plot the right symbols
[feas_ind, feas] = find(y_cnst == 1);
[infeas_ind, infeas] =find(y_cnst == -1);
%plot3(ax2,x_cnst(:,1), x_cnst(:,2),y_cnst,'.r','MarkerSize',20)
scatter3(ax2, x_cnst(feas_ind,1),x_cnst(feas_ind,2),y_cnst(feas_ind)+3,50,'MarkerFaceColor','w','LineWidth',0.9,'MarkerEdgeColor','k')
scatter3(ax2, x_cnst(infeas_ind,1),x_cnst(infeas_ind,2),y_cnst(infeas_ind)+3,50,'MarkerEdgeColor','w','LineWidth',0.7,'MarkerFaceColor','k')
scatter3(ax2, min_loc(1),min_loc(2),5, 30,'pentagram','MarkerEdgeColor','y','LineWidth',2,'MarkerFaceColor','none')
